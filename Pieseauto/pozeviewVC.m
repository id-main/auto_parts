...
@interface pozeviewVC(){
    BOOL newMedia;
}
@end

@implementation pozeviewVC
@synthesize faopoza, ttil, POZEALESE,POZEUTILIZATOR,librarie,numarmaxpoze,libraryx,pozaNOUA,ARRAYASSETURI,ARRAYASSETURIEXTERNE;
...
-(void)preiaPOZEALESE {
    ...
    if(  del.reposteazacerere ==YES) {
        del.ARRAYASSETURIEXTERNE = del.ARRAYASSETURI;
...
-(void)POZEIOS7 {
    ...
    if(self.ARRAYASSETURIEXTERNE && self.ARRAYASSETURIEXTERNE.count>0) {
        [collector addObjectsFromArray:self.ARRAYASSETURIEXTERNE];
    }
...
//         [self removehud];
//        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Eroare"
//                                                            message:@"Permiteți aplicației să utilizeze galeria foto"
//                                                           delegate:nil
//                                                  cancelButtonTitle:@"Ok"
//                                                  otherButtonTitles:nil];
//        [alertView show];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    [self addhud];
      if(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")){
       //  [self performSelector:@selector(poze8) withObject:nil afterDelay:3.0];

        self.POZEUTILIZATOR = [self mo8];
        [self forcereload];
    } else {
       [self POZEIOS7];
      //   [self performSelector:@selector(poze7) withObject:nil afterDelay:3.0];
    }
}
...
- (void)viewDidLoad {
    ...
    NSLog(@" self.POZEALESE %@", self.POZEALESE);
    AppDelegate *del = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    if(del.POZECERERE.count >0) {
        self.POZEALESE = del.POZECERERE;
    } else {
        self.POZEALESE = [[NSMutableArray alloc]init];
    }
    if(del.ARRAYASSETURI.count >0) {
        self.ARRAYASSETURI = del.ARRAYASSETURI;
    } else {
        self.ARRAYASSETURI = [[NSMutableArray alloc]init];
    }
    if(del.ARRAYASSETURIEXTERNE.count >0) {
        self.ARRAYASSETURIEXTERNE = del.ARRAYASSETURIEXTERNE;
    } else {
        self.ARRAYASSETURIEXTERNE = [[NSMutableArray alloc]init];
    }
    NSLog(@"ARRAYASSETURIEXTERNE %@",ARRAYASSETURIEXTERNE);
    
    
    pozaNOUA =NO;
    NSLog(@"poze view");
    NSUserDefaults* prefs=[NSUserDefaults standardUserDefaults];
    if([prefs objectForKey:@"CERERE_max_images"]) {
        numarmaxpoze =[[prefs objectForKey:@"CERERE_max_images"]intValue];
    } else {
        numarmaxpoze =4;
    }
    // testat cu    numarmaxpoze =20; e ok
    NSLog(@"nr max poze %@ %i",[prefs objectForKey:@"CERERE_max_images"], numarmaxpoze);
...
-(NSMutableArray*)mo8 {
    ...
    // return xtest;
    if(self.ARRAYASSETURIEXTERNE && self.ARRAYASSETURIEXTERNE.count>0) {
        [finala addObjectsFromArray:self.ARRAYASSETURIEXTERNE];
    }
...
/* gunoi
 ...
 ///   __block UIImage *ima;
 //tumbnailuri si asseturi
 
 for (PHAsset *asset in allPhotosResult) {
 if(![ xtest containsObject:asset])
 [xtest addObject:asset];
 ...
 //                            NSLog(@"immag 8 %@",ima);
 //                           // [xtest addObject:ima];
 //                            [self.THUMBNAILSPOZEUTILIZATOR addObject:ima];
 //                        }];
 }
 ///   NSLog(@"self.THUMBNAILSPOZEUTILIZATOR  %@",self.THUMBNAILSPOZEUTILIZATOR );
 ...
 //            [manager requestImageForAsset:asset
 //                            //   targetSize:PHImageManagerMaximumSize
 //               targetSize:CGSizeMake(self.view.frame.size.width/3.12f,self.view.frame.size.width/3.12f)
 //                              contentMode:PHImageContentModeDefault
 //                                  options:requestOptions
 //                            resultHandler:^void(UIImage *image, NSDictionary *info) {
 //                                ima = image;
 //                                NSLog(@"immag 8 %@",ima);
 //                                if(![xtest containsObject:image]) {
 //                                [xtest addObject:ima];
 //                                }
 //                            }];
 //             }
...
//                                     contentMode:PHImageContentModeAspectFill
//                                         options:nil];
//// [xtest insertObject:@"butonmarefapoza.png" atIndex:0]; //finaly
//return xtest;
//
//}
//
